                  ...
                  <tr key={order._id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-black uppercase tracking-widest text-gray-600 bg-gray-100 px-2 py-1 rounded">
                        {order.paymentMethod === 'ONLINE_QR' ? 'Online QR' : order.paymentMethod === 'Razorpay' ? 'Online' : 'COD'}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <div className="flex flex-col space-y-2">
                          {!order.isPaid && order.paymentMethod === 'ONLINE_QR' && !order.isCancelled && (
                            <button 
                              onClick={() => markAsPaidHandler(order._id)}
                              disabled={actionLoading === order._id}
                              className="flex items-center space-x-1 justify-center text-[9px] font-black uppercase tracking-widest text-orange-600 border border-orange-600 px-3 py-1.5 rounded-xl hover:bg-orange-600 hover:text-white transition-all disabled:opacity-50"
                            >
                              {actionLoading === order._id ? <Loader2 size={10} className="animate-spin" /> : <CheckCircle2 size={10} />}
                              <span>Paid</span>
                            </button>
                          )}
                          {!order.isDelivered && !order.isCancelled && (
                            <button 
                              ...
                              disabled={actionLoading === order._id}
                              className="flex items-center space-x-1 justify-center text-[9px] font-black uppercase tracking-widest text-green-600 border border-green-600 px-3 py-1.5 rounded-xl hover:bg-green-600 hover:text-white transition-all disabled:opacity-50"
                            >
                              {actionLoading === order._id ? <Loader2 size={10} className="animate-spin" /> : <PackageCheck size={10} />}
                              <span>Delivered</span>
                            </button>
                          )}
                        </div>
